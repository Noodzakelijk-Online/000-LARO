...
// Create a persisted query cache
const persistedQueryCache = new PersistedQueryCache({
  // Use in-memory cache for development, would use Redis in production
  cache: {
    get: async (key) => {
      return persistedQueries[key] || null;
    },
    set: async (key, value) => {
      persistedQueries[key] = value;
      return true;
    }
  }
});

// In-memory storage for persisted queries
...
// Handle persisted queries endpoint
const http = require('http');
const url = require('url');

// Create a simple HTTP server for persisted queries
const persistedQueryServer = http.createServer((req, res) => {
  if (req.method === 'POST' && req.url === '/graphql/persisted') {
    let body = '';
    ...
    req.on('end', () => {
      try {
        ...
        if (!data.id) {
          res.statusCode = 400;
          res.end(JSON.stringify({ errors: [{ message: 'No persisted query ID provided' }] }));
          return;
        }
        
        const query = persistedQueries[data.id];
        
        if (!query) {
          res.statusCode = 404;
          res.end(JSON.stringify({ errors: [{ message: 'Persisted query not found' }] }));
          return;
        }
        
        // Forward to Apollo Server
        const apolloReq = http.request({
          hostname: 'localhost',
          port: 4000,
          path: '/graphql',
          method: 'POST',
          ...
          apolloRes.on('data', chunk => {
            apolloBody += chunk.toString();
          });
          
          apolloRes.on('end', () => {
            res.statusCode = apolloRes.statusCode;
            res.end(apolloBody);
          });
        });
        
        apolloReq.write(JSON.stringify({
          query,
...
persistedQueryServer.listen(4001, () => {
  console.log('Persisted query server listening on port 4001');
});
