...
import {
  ...
  syncGoogleDriveForCase,
  connectGoogleDrive,
  getGoogleDriveStatus,
  disconnectGoogleDrive,
} from '../googleDriveServiceEnhanced';
 ...
 * - Status tracking
 */
export const googleDriveEnhancedRouter = router({
  /**
  ...
  getOAuthUrl: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        const authUrl = getAuthorizationUrl('gmail', ctx.user.id);
  ...
  connect: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const sourceId = await connectGoogleDrive(
          ctx.user.id,
          input.caseId,
          input.accessToken
        );
  ...
  startSync: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const progress = await syncGoogleDriveForCase(
          ctx.user.id,
          input.caseId,
          accessToken,
          input.sourceId
  ...
  getStatus: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        const status = await getGoogleDriveStatus(input.caseId);
      ...
      try {
        ...
        const source = await db
          ...
          .where(
            and(
              eq(evidenceSources.caseId, input.caseId),
              eq(evidenceSources.sourceType, 'Google Drive')
            )
          )
          .limit(1);
        ...
        const result = await listAllGoogleDriveFiles(
          accessToken,
          input.folderId,
          input.pageToken
        );
  ...
  listFolders: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        const result = await getAllGoogleDriveFolders(
          accessToken,
  ...
  disconnect: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        await disconnectGoogleDrive(input.sourceId);
